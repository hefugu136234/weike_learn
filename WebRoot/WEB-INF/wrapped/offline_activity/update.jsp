				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<select id="enrollType" name="enrollType" class="form-control"
							required="required">
							<option value="0"
								${vo.enrollType eq 0 ? ' selected="selected"':''}>公开</option>
							<option value="1"
								${vo.enrollType eq 1 ? ' selected="selected"':''}>审核</option>
						</select>
					</div>
				</div>
				<div class="hr-line-dashed"></div>
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<input type="text" class="form-control" name="limitNum"
							maxlength="8" required="required" value="${vo.limitNum}" />
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<input type="text" class="form-control" id="price" name="price"
							maxlength="8" required="required" value="0" />
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<input type="text" class="form-control" name="integral"
							maxlength="8" required="required" id="integral" value="0" />
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<textarea class="form-control" rows="4" name="mark"
							required="required" maxlength="249" placeholder="简介(200字以内)">${vo.mark}</textarea>
					</div>
				</div>
				<div class="hr-line-dashed"></div>
				...
				<div class="form-group">
					...
					<div class="col-md-8">
						...
						<script id="description_script" name="description"
							type="text/plain" style="height: 350px;">${vo.description}</script>
					</div>
				</div>
				
				<input type="hidden" id="uuid" name="uuid" value="${vo.uuid}" />
				<textarea class="hide" id="cover" name="cover">${vo.cover}</textarea>
				<textarea class="hide" id="price_json">${vo.price}</textarea>
...
<script type="text/javascript">
	...
	$(function(){
		cover_json = $('#cover').text();
		 price_json = $('#price_json').text();
			price_json = $.parseJSON(price_json);
			$('#price').val(price_json.price);
			$('#integral').val(price_json.integral);
		
		try {
			cover_json = $.parseJSON(cover_json);
		} catch (err) {
			console.log(err);
		}
		...
		$('input[type="file"]').each(function(i, e) {
			...
			uploaderInit(new Part($e, $e.data("type"), function(part) {
				if(typeof cover_json === 'object'){
					$.each(cover_json, function(i, e) {
						if (part.type == e.type) {
	...
	function submitFrom(from) {
		...
		$.post('/admin/offline/data/update', $(from).serialize(), function(data) {
