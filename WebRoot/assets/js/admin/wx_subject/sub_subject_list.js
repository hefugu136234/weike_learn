...
var wxSubject_sub_list_table,subject_modal,confirm_subject,subject_tree,wx_subject_name,linkType,cover_div,parent_uuid;
var current_node_uuid='';
var select_trace_text='';
var $floor_modal,$subject_parent,$floor_confirm;
$(function() {
	...
	$floor_modal=$('#floor_modal');
	$subject_parent=$('#subject_parent');
	$floor_confirm=$('#floor_confirm');
	// 加载table数据
	wxSubject_sub_list_table=$('#wxSubject_sub_list_table').dataTable({
		"bProcessing" : true,
		"bServerSide" : true,
		"bStateSave" : false,
		...
		"iDisplayLength" : 10,
		"iDisplayStart" : 0,
		"sAjaxSource" : "/project/wx/subject/sub/list/data/"+parent_uuid+"?rootType=1",
		"aoColumns" : [ {
			"mData" : "name",
			"orderable" : false,
				...
				'parentUuid':parent_uuid
					...
					function(data){
						if(data.status=='success'){
							...
							wxSubject_sub_list_table.fnDraw(false);
						}else{
							alert(data.message);
						}
					});
...
function loadAddLink(cell, uuid, isStatus,propertype) {
	...
	//层级修改
	var floor_change=$('<a style="margin-left:10px" href="javascript:void(0);">层级变更</a>');
	floor_change.click(function(event){
		event.preventDefault();
		$subject_parent.empty();
		$subject_parent.append('<option value="">请选择</option>');
		$.getJSON('/project/wx/subject/get/abled/parent',{
			'uuid':uuid
		},function(data){
			var s_index=0;
			if(data.status=='success'){
				...
				if(!!items&&items.length>0){
					$.each(items,function(index,item){
						var s_item='<option value="'+item.uuid+'">'+item.name+'</option>';
						$subject_parent.append(s_item);
						if(item.parent){
							s_index=index+1;
						}
					});
			...
			$subject_parent.get(0).selectedIndex=s_index;
		});
		$floor_confirm.unbind('click');
		$floor_confirm.click(function(e){
			e.preventDefault();
			var parent_select_uuid=$subject_parent.val();
			if(parent_select_uuid==''){
				alert('请选中父目录');
				return ;
			}
			$.post('/project/wx/subject/change/parent',{
				'uuid':uuid,
				'parentuuid':parent_select_uuid
			},function(data){
				if(data.status=='success'){
					$floor_modal.modal('hide');
					wxSubject_sub_list_table.fnReloadAjax("/project/wx/subject/sub/list/data/"+parent_uuid+"?rootType=1");
				}else{
					alert(data.message);
				}
			});
