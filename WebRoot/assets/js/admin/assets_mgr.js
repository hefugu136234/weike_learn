...
$(document).ready(function() {
	...
	$('#video_new_form [type="submit"]').click(function(e) {
		e.preventDefault();
		var videotype_val=$('#videotype').val();
		var fileId=$('#fileId').val();
		var qiuniu_key=$('#qiuniu_key').val();
		if(videotype_val=='0'&&!fileId){
			alert('请先上传文件')
			return;
		}
		if (videotype_val=='1'&&!qiuniu_key) {
			alert('请先上传文件')
			return;
		}
		if (!$('#categoryUuid').val()) {
		...
		// $('#video_new_form').submit();
		$.post('/asset/video/save',$('#video_new_form').serialize()).always(function(data) {
			console.log(data)
			if (data.status == 'success') {
				alert('保存成功，请设置该资源的访问权限');
				location.href = '/asset/video/mgr'
