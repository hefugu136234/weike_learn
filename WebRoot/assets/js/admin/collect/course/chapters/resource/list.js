...
$(document).ready(
		function() {
			...
			dataTable = datatable($('#collect_chapters__res_list_table'),
					'/project/collect/course/chapters/resource/list/data/'
							+ chapterUuid, [ name, mark, _status, operator ])
		})
...
function oprationCell(nTd, sData, oData, rows) {
	...
	item.click(function() {
		$.post('/project/collect/course/chapters/resource/status', {
			uuid : oData['uuid']
		}, function() {
		}).done(function() {
	...
	edit.click(function() {
		window.location.href = "/project/collect/course/chapters/resource/update/page/"
				+ oData['uuid'];
	})
	nTd.append(edit);
	...
	remove.click(function() {
		if (confirm("是否要删除该资源?")) {
			$.post('/project/collect/course/chapters/resource/remove', {
						uuid : oData['uuid']
				}, function() {
				})
				.done(function() {
				...
				.always(function(data) {
					if (data.status == 'success') {
						dataTable.table
								.fnReloadAjax('/project/collect/course/chapters/resource/list/data/'
										+ chapterUuid);
					} else {
						if (!!data['message']) {
							alert(data['message']);
	...
	top.click(function() {
		$.post('/project/collect/course/chapters/resource/top', {
			uuid : oData['uuid']
		}, function() {
		}).done(function() {
