...
$(function(){
	...
	$('#exchange_code_table').dataTable({
		...
		"iDisplayLength" : 10,
		"iDisplayStart" : 0,
		"sAjaxSource" : "/admin/offline/type/code/list/data/"+uuid+"?type=2",
		"aoColumns" : [ {
			"mData" : "projectCode",
			"orderable" : false
		}, {
	...
	$('#code_confirm').click(function(e){
		...
		$.post('/admin/offline/creat/type/code',{
			...
			if(data.status=='success'){
				$('#exchange_code_table').dataTable().fnDraw(false);
...
function statusCell(cell, isStatus) {
	...
	var value = '';
	 if (isStatus == 1) {
		value = '<span style="color: green;">已兑换</span>';
	} else if (isStatus == 0) {
		value = '<span style="color: red;">未兑换</span>'
	}
	_p.append(value);
}