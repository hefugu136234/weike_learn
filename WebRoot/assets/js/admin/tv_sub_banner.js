...
$(function() {
	var count = 0;
	// 第一次进来加载首页
	// $.get('/tv/home/structure1?timestamp=' + new Date().getTime(), function()
	// {
	...
	if (widgets.length > 0) {
		$(widgets).each(function(i, e) {
			count = Math.max(parseInt(e.scriptId.split('-')[1]), count);
			console.log(e)
			addOrUpdateWidget(e);
			init_widget_ui(e)
		})
	...
	function init_widget_ui(widget) {
		var w = $('<li class="widget-item" id="'
				...
				+ widget.scriptId
				+ '" style="display:none" class="gs_setting">设置</a> <span style="display:none;" class="gs_remove_handle">x</span></li>');
		...
		if (widget.categoryId) {
			...
			child.click(function(e) {
				e.preventDefault();
				window.location.href = "/tv/layout/" + widget.categoryId
						+ "/list";
			})
		}
	...
	$('#settingModal').on(
			...
			function(e) {
				...
				if (widget_obj) {
					...
					// console.log(widget_obj.categoryId)
					// 获取当前的选择
	...
	$('#save_widgets_btn').click(
			function() {
				...
				if (ok) {
					$.post("/tv/layout/" + category.uuid + "/save?timestamp=" + new Date().getTime(), {
						widgets : JSON.stringify(data),
						name : $('#layout_name').val(),
						mark : $('#layout_mark').val(),
						uuid : $('#layout_uuid').val()
					}).done(function(data) {						
						if(data.status == 'success'){
							alert('更新成功')
							window.location.href = '/tv/layout/'+category.uuid+'/list'
						}else{
							alert('更新失败,请尝试重新提交')
						}
					}).fail(function() {
