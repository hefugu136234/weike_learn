...
function loadDate_cloud(){
	// 加载table数据
	table_cloud = $('#bannerTable_cloud').dataTable({
		"bProcessing" : true,
		"bServerSide" : true,
		"bStateSave" : false,
		...
		"iDisplayLength" : 10,
		"iDisplayStart" : 0,
		"sAjaxSource" : "/project/banner/showBannerList?type=1",
		...
		"aoColumns" : [ {
			...
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				showImages_cloud(nTd, sData, '预览');
			}
		}, {
			"mData" : "title",
			"orderable" : false,
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				showDetail_cloud(nTd, sData);
			}
		},{
			"mData" : "createDate",
			"orderable" : false
			...
			"orderable" : false,
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				showType_cloud(nTd, sData);
			}
		}, {
			"mData" : "state",
			"orderable" : false,
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				showStatus_cloud(nTd, sData);
			}
		}, {
			"mData" : "validDate",
			"orderable" : false,
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				validDateDetail_cloud(nTd, sData);
			}
		} ,{
			"mData" : "mark",
			"orderable" : false,
			...
			"orderable" : false,
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				operation_cloud(nTd, sData, oData, oData['state']);
			}
		}]
	});
}
...
function showImages_cloud(cell, taskId, title) {
	var parent = $(cell);
	parent.empty();
	if (!!taskId) {
...
//Banner 平台类型展示
function showType_cloud(nTd, type){
	var _p = $(nTd);
	_p.empty();
	var value = '';
	if (type == '1') {
...
//Banner 当前状态显示
function showStatus_cloud(nTd, state){
	var _p = $(nTd);
	_p.empty();
	var value = '';
	if (state == '2') {
...
//标题，RefUrl, 备注等信息完整显示
function showDetail_cloud(td, data){
	var td = $(td);
	td.empty();
	if(data.length > 10){
	...
	td.on("click", function(){
		$('#dataDetail_cloud').text(data);
		$('#dataModal_cloud').modal('show');
	})
}

//有效期详情显示
function validDateDetail_cloud(nTd, sData){
	var td = $(nTd);
	td.empty();
	var value = sData;
	if(0 == sData){
...
//上下线，编辑, 软删除等操作
function operation_cloud(td, uuid, data, state){
	var parent = $(td);
	parent.empty();
	var stateOperation = '';
	if (state == '2'){
...
//点击上下线操作后，显示图片当前的状态
function refreshItem_cloud(row, data) {
	showStatus_cloud(row[6], data['state']);
	operation_cloud(row[9], data['uuid'], data, data['state']);
}
