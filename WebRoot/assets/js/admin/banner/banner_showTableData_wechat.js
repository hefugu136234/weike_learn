...
function loadDate_wechat(){
	// 加载table数据
	table_wechat = $('#bannerTable_wechat').dataTable({
		"bProcessing" : true,
		"bServerSide" : true,
		"bStateSave" : false,
		...
		"iDisplayLength" : 10,
		"iDisplayStart" : 0,
		"sAjaxSource" : "/project/banner/showBannerList?type=3",
		...
		"aoColumns" : [ {
			...
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				showImages_wechat(nTd, sData, '预览');
			}
		}, {
			"mData" : "title",
			"orderable" : false,
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				showDetail_wechat(nTd, sData);
			}
		},{
			"mData" : "createDate",
			"orderable" : false
			...
			"orderable" : false,
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				showType_wechat(nTd, sData);
			}
		}, {
			"mData" : "state",
			"orderable" : false,
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				showStatus_wechat(nTd, sData);
			}
		}, {
			"mData" : "validDate",
			"orderable" : false,
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				validDateDetail_wechat(nTd, sData);
			}
		} ,{
			"mData" : "mark",
			"orderable" : false,
			...
			"orderable" : false,
			"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
				operation_wechat(nTd, sData, oData, oData['state']);
			}
		}]
	});
}
...
function showImages_wechat(cell, taskId, title) {
	var parent = $(cell);
	parent.empty();
	if (!!taskId) {
...
//Banner 平台类型展示
function showType_wechat(nTd, type){
	var _p = $(nTd);
	_p.empty();
	var value = '';
	if (type == '1') {
...
//Banner 当前状态显示
function showStatus_wechat(nTd, state){
	var _p = $(nTd);
	_p.empty();
	var value = '';
	if (state == '2') {
...
//标题，RefUrl, 备注等信息完整显示
function showDetail_wechat(td, data){
	var td = $(td);
	td.empty();
	if(data.length > 10){
	...
	td.on("click", function(){
		$('#dataDetail_wechat').text(data);
		$('#dataModal_wechat').modal('show');
	})
}

//有效期详情显示
function validDateDetail_wechat(nTd, sData){
	var td = $(nTd);
	td.empty();
	var value = sData;
	if(0 == sData){
...
//上下线，编辑, 软删除等操作
function operation_wechat(td, uuid, data, state){
	var parent = $(td);
	parent.empty();
	var stateOperation = '';
	if (state == '2'){
...
//点击上下线操作后，显示图片当前的状态
function refreshItem_wechat(row, data) {
	showStatus_wechat(row[6], data['state']);
	operation_wechat(row[9], data['uuid'], data, data['state']);
}
